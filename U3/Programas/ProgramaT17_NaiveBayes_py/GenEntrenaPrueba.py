...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def createFiles(self):
        ...
        archivo = open("cancer.csv", "r")  # ABRE EL ARCHIVO
        ...
        lista = [linea.replace(" ","").split(",") for linea in contenido]

        instanica = []
        for data in lista:
            aux = list(map(int, data[:9]))
            aux.append((data[9].replace("\n","")))
        ...
        fileEntrena = open("Instancias/Cancer_training7030.csv", "w")
        filePrueba = open("Instancias/Cancer_test7030.csv", "w")
